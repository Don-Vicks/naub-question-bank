    ...
    <div className="page-desktop">
      <div className="page-header lg:rounded-card-xl lg:mx-0 lg:my-6">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="page-header-sub">
            {paper.examType} · {paper.session} · {paper.pageCount} pages
          </p>
        </div>
      </div>
      ...
      <div className="content-area">
        <div className="mx-auto max-w-2xl">
          {hasPages ? (
            <>
              <div className="relative overflow-hidden rounded-card-xl border border-line bg-white shadow-card animate-fade-in">
                ...
                <div className="absolute right-3 top-3">
                  <button
                    ...
                    className="flex h-9 w-9 items-center justify-center rounded-xl bg-ink/70 text-paper backdrop-blur-sm transition-all hover:bg-ink/90"
                    aria-label="Zoom in"
                  >
                  ...
                  {pages.map((_, i) => (
                    <button
                      ...
                      className={`h-1.5 rounded-full transition-all duration-200 ${
                        i === currentPage ? 'w-6 bg-naub-green' : 'w-1.5 bg-ink/15 hover:bg-ink/25'
                      }`}
                    />
                  ))}
                </div>
                    ...
                    <button
                      ...
                      className={`flex-shrink-0 overflow-hidden rounded-xl border-2 transition-all duration-200 ${
                        i === currentPage
                          ...
                          : 'border-line opacity-50 hover:opacity-100 hover:border-ink/20'
                      }`}
                    >
            ...
            <div className="rounded-card-xl border border-line bg-white p-12 text-center">
              <p className="text-body text-muted">
                {paper.status === 'extracting' || paper.status === 'splitting'
                  ? 'This paper is still being processed. Pages will appear here once ready.'
                  : paper.status === 'failed'
                    ? 'Processing failed. Please try uploading again.'
                    : 'No pages available for this paper.'}
              </p>
            </div>
          )}
        </div>
          ...
          {pages.length > 1 && (
            <div className="absolute bottom-6 left-0 right-0 flex items-center justify-center gap-6">
              <button
                ...
                disabled={currentPage === 0}
                className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/10 text-paper backdrop-blur-sm transition-all hover:bg-white/20 disabled:opacity-30"
              >
